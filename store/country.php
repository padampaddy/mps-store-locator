 ...
 */
function call_CountryClass()
{
    new CountryClass();
}

if (is_admin()) {
    add_action('load-post.php',     'call_CountryClass');
    add_action('load-post-new.php', 'call_CountryClass');
}
...
class CountryClass
{
    public $name = 'mps_stores_country';
...
?>
        <input id="<?= $this->name ?>" name="<?= $this->name ?>" value="<?= $value ?>" style="width:100%;height:40px;border:none;" />
<?php
    }
}
